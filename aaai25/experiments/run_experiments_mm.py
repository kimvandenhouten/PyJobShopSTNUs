...
import numpy as np
import pandas as pd
from mmrcpsp_wd.solvers.MMRCPSP_CP_benchmark import MMRCPSP_CP_Benchmark
import ast
import copy
import datetime
...
# GENERAL SETTINGS
SEED = 1
DIRECTORY_INSTANCES = os.path.join('mmrcpsp_wd', 'data', 'instances_with_deadlines')
...
for noise_factor in NOISE_FACTORS:
    output_file = f'final_results_{noise_factor}_{now}_mm.csv'
    data = []
    for instance_folder in INSTANCE_FOLDERS:
        for file in os.listdir(os.path.join(DIRECTORY_INSTANCES, instance_folder)):
            path = os.path.join(DIRECTORY_INSTANCES, instance_folder, file)
            # PREPARE DATA AND DURATION SAMPLES
            mmrcpsp = MMRCPSP_CP_Benchmark.parsche_file(path, noise_factor, instance_folder)
            np.random.seed(SEED)
            test_durations_samples = mmrcpsp.sample_durations(nb_scenarios_test)

            # RUN PROACTIVE / REACTIVE OFFLINE
            data_dict_baseline = run_proactive_offline(mmrcpsp, time_limit_proactive, mode_proactive, nb_scenarios_saa)
            ...
            if stnu:
                mmrcpsp = MMRCPSP_CP_Benchmark.parsche_file(path, noise_factor, instance_folder)
                dc, estnu, data_dict_stnu = run_stnu_offline(mmrcpsp, time_limit_cp_stnu=time_limit_cp_stnu, mode=mode_stnu)

                for i, duration_sample in enumerate(test_durations_samples):
                    # pi_feasible, obj_pi = check_pi_feasible(instance_folder, instance_id, i, duration_sample, noise_factor)
                    # data_dict_stnu["obj_pi"] = obj_pi
                    if True:
                        data += run_stnu_online(dc, estnu, duration_sample, mmrcpsp, data_dict_stnu)
                        data_df = pd.DataFrame(data)
                        if writing:
                            data_df.to_csv(output_file, index=False)
                    else:
                        logger.info(f'Instance {mmrcpsp.instance_folder}PSP{mmrcpsp.instance_id}, sample {i}:'
