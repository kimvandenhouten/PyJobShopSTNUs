...
if proactive:
    ...
    for (mode, time_limit, nb_scenarios_saa) in [("quantile_0.9", 60, 1)]:
        # Run the experiments
            ...
            for instance_id in INSTANCE_IDS:
                ...
                for i, duration_sample in enumerate(test_durations_samples):
                    ...
                    if pi_feasible:
                        ...
                        if writing:
                            data_df.to_csv(f'experiments/aaai25_experiments/results/proactive_vs_reactive_with_warmstart.csv', index=False)
                    else:
...
for (mode, time_limit_initial, time_limit_rescheduling) in [("quantile_0.5", 60, 2)]:
    # Run the experiments
    for instance_folder in INSTANCE_FOLDERS:
        for instance_id in INSTANCE_IDS:
            np.random.seed(SEED)
